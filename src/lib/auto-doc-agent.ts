...
Your final response MUST be raw markdown only — no preamble, no explanation, no meta-commentary. Start directly with the first heading or paragraph of the documentation. Do not write sentences like "Here is the documentation" or "I have gathered everything I need." Output only the content that will be saved to the file.${
    selectedPaths && selectedPaths.length > 0
      ? `\n\nThe user has selected these files and folders for you to focus on. Prioritize reading these files — you do not need to use list_repo_tree first if the list is sufficient:\n\n${selectedPaths.map((p) => `- ${p}`).join('\n')}`
      : ''
  }`;
  ...
  while (iterations++ < MAX_ITERATIONS) {
    ...
    for (const block of response.content) {
      ...
      if (block.name === 'list_repo_tree') {
        if (treeCallCount >= MAX_TREE_CALLS) {
          result = 'Repository exploration limit reached.';
          wrapUp = true;
        } else {
          treeCallCount++;
        ...
        if (readFileCount >= MAX_READ_FILES) {
          result = 'File read limit reached.';
          wrapUp = true;
        } else {
          readFileCount++;
  ...
  // Limit reached — ask the model to write up what it has gathered
  onProgress('Wrapping up...');
  messages.push({
    role: 'user',
    content:
      'You have reached the maximum number of research steps. Write the documentation now based on everything you have gathered so far.',
  });
